...
import { Trophy, Mail, Lock, User, Sparkles, ArrowRight, Shield, MapPin, Navigation, Loader2 } from "lucide-react";
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signin") {
        ...
        const { error } = await supabase.auth.signUp({
          ...
          options: {
            data: {
              ...
              city,
              uid: "MATE-" + Math.random().toString(36).substring(2, 8).toUpperCase(),
            },
          },
        });
        ...
        toast.success("Account created successfully! Welcome to GamesMate! 🎉", {
          style: {
            background: "var(--brand-green)",
            color: "var(--violet-deep)",
            border: "2px solid var(--gold)",
          ...
          <div className="panel-game p-8">
            <div className="relative z-10">
              ...
              <form onSubmit={handleAuth} className="space-y-4">
                ...
                {mode === "signup" && (
                  <div>
                    <label className="block text-xs font-bold text-[color:var(--card-foreground)] mb-1 flex items-center justify-between">
                      <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5" /> City Location</span>
                      <button 
                        type="button" 
                        onClick={fetchLiveLocation}
                        disabled={locating}
                        className="text-[10px] bg-[var(--brand-yellow)]/20 hover:bg-[var(--brand-yellow)]/60 text-[var(--violet-deep)] px-2 py-0.5 rounded flex items-center gap-1 transition font-extrabold"
                      >
